		...
		client.connect(TOKEN, new Nandbox.Callback() {
			...
			public void onReceive(IncomingMessage incomingMsg) {
				...
				else
				{
					...
					System.out.println(
							"incomingMsg.getFrom().getStatus(): " + incomingMsg.getFrom().getStatus());
					System.out.println("incomingMsg.getFrom().getVersion() : "
							+ incomingMsg.getFrom().getVersion());
					System.out.println("incomingMsg.getFrom().getType() : "
							+ incomingMsg.getFrom().getType());					
					...
					if (MessageType.text.toString().equals(incomingMsg.getType())) {
						...
						else if (incomingMsg.getText().equals("getUser"))
						{
							...
							Button oneBtn = createButton("", "oneBtnCBInWebView", 1,
									"RED", "White", null, null);
							...
							Button oneBtn = createButton("Visit a Milestone", "oneBtnCBInWebView", 1,
									"RED", "White", null, null);
							Button secondBtn = createButton("Cairo Porto Mall", "secondBtn", 1,
									"RED", "White", null, null);
							Button thirdButton = createButton("Seven Stars Mall", "thirdBtn", 1,
									"RED", "White", null, null);							
							...
							outmsg.setEcho(1);
							String menuRef = MAIN_MENU_001;
							Button oneBtn = createButton("RSS", "oneBtnCBInWebView", 1,
									"RED", "White", null, null);
							oneBtn.setButtonIcon("ic_mood_bad_24dp");
							oneBtn.setButtonIconBgColor("#FFFF44");							
							Button secondBtn = createButton("Calendar", "secondBtn", 1,
									"RED", "White", null, null);
							secondBtn.setButtonIcon("ic_hourglass_full_24dp");
							secondBtn.setButtonIconBgColor("White");	
							Button thirdButton = createButton("Feed", "thirdBtn", 1,
									"RED", "White", null, null);		
			...
			private void handleIncomingDocumentMsg(IncomingMessage incomingMsg) {
				...
				System.out.println(
						"incomingMsg.getDocument().getId() : " + incomingMsg.getDocument().getId());
				System.out.println("incomingMsg.getDocument().getName() : "
						+ incomingMsg.getDocument().getName());
				System.out.println("incomingMsg.getDocument().getSize() : "
						+ incomingMsg.getDocument().getSize());

				DocumentOutMessage documentOutMsg = new DocumentOutMessage();
				documentOutMsg.setChatId(incomingMsg.getChat().getId());
				documentOutMsg.setReference(
						getUniqueId());
			...
			//	String uploadedDocumentId = MediaTransfer.uploadFile(TOKEN,
				...
				api.sendDocument(incomingMsg.getChat().getId(), uploadedDocumentId,
						"Document Caption");
				api.sendDocument(incomingMsg.getChat().getId(), uploadedDocumentId,
						getUniqueId(),
						"Send doc with ref");
				api.sendDocument(incomingMsg.getChat().getId(), uploadedDocumentId,
						getUniqueId(), null, null, null,
						null, "from all option send", null, null, null);

				api.sendText(incomingMsg.getChat().getId(),
						"Document size : " + incomingMsg.getDocument().getSize()
			...
			private void handleIncomingContactMsg(IncomingMessage incomingMsg) {
				...
				System.out.println("incomingMsg.getContact()getName() : "
						+ incomingMsg.getContact().getName());
				System.out.println("incomingMsg.getContact().getPhoneNumber() : "
						+ incomingMsg.getContact().getPhoneNumber());
				...
				api.sendContact(incomingMsg.getChat().getId(),
						incomingMsg.getContact().getPhoneNumber(),
						incomingMsg.getContact().getName());
				...
				api.sendText(incomingMsg.getChat().getId(),
						" Contact Name  is : " + incomingMsg.getContact().getName()
								+ " Phone number  is : "
								+ incomingMsg.getContact().getPhoneNumber());
			...
			private void handleIncomingTextFileMsg(IncomingMessage incomingMsg) {
				...
				System.out.println("incomingMsg.getTextFile().getSize(): "
						+ incomingMsg.getTextFile().getSize());

				MediaTransfer.downloadFile(TOKEN, textFileId, "./download", null);

				String uploadedTextFileId = MediaTransfer.uploadFile(TOKEN,
						"./download/" + textFileId);

				api.sendDocument(incomingMsg.getChat().getId(), uploadedTextFileId,
						getUniqueId(), null, null, null,
			...
			private void handleIncomingLocationMsg(IncomingMessage incomingMsg) {
				...
				System.out.println("incomingMsg.getLocation().getName() : "
						+ incomingMsg.getLocation().getName());
				System.out.println("incomingMsg.getLocation().getDetails() : "
						+ incomingMsg.getLocation().getDetails());
				System.out.println("incomingMsg.getLocation().getLatitude() : "
						+ incomingMsg.getLocation().getLatitude());
				System.out.println("incomingMsg.getLocation().getLongitude() : "
				...
				api.sendlocation(incomingMsg.getChat().getId(),
						incomingMsg.getLocation().getLatitude(),
						incomingMsg.getLocation().getLongitude());
				api.sendlocation(incomingMsg.getChat().getId(),
						incomingMsg.getLocation().getLatitude(),
				...
				api.sendText(incomingMsg.getChat().getId(),
						" Latitude is : " + incomingMsg.getLocation().getLatitude()
								+ " Longitude is : " + incomingMsg.getLocation().getLongitude()
								+ " and name is :" + incomingMsg.getLocation().getName()
								+ " and details is :" + incomingMsg.getLocation().getDetails());
			}
			...
			private void handleIncomingGifMsg(IncomingMessage incomingMsg) {
				...
				System.out.println(
						"incomingMsg.getGif().getWidth(): " + incomingMsg.getGif().getWidth());
				System.out.println(
						"incomingMsg.getGif().getHeight(): " + incomingMsg.getGif().getHeight());
				System.out.println(
						"incomingMsg.getGif().getSize(): " + incomingMsg.getGif().getSize());
				System.out.println(
						"================start of Gif Thumbinil  Object ===================");
				if (incomingMsg.getGif().getThumbnail() != null
						&& incomingMsg.getGif().getId().endsWith(".gif")) {
					System.out.println(
							"================End of Gif Thumbinil Object ===================");
					...
					api.sendText(incomingMsg.getChat().getId(),
							"Gif Size is : " + incomingMsg.getGif().getSize()
									+ " and Gif width is :" + incomingMsg.getGif().getWidth()
									+ " and Gif height is :" + incomingMsg.getGif().getHeight()
									+ " and caption is : " + incomingMsg.getCaption()
									+ "\n\n Wait please sending you a Gif ....");

					String uploadedGifPhotoId = MediaTransfer.uploadFile(TOKEN,
							"./upload/gif_sample.gif");
					...
					String uploadedGifVideoId = MediaTransfer.uploadFile(TOKEN,
					...
					if (uploadedGifVideoId != null) {
						...
						api.sendGIFVideo(incomingMsg.getChat().getId(), uploadedGifVideoId,
								"without ref");
						api.sendGIFVideo(incomingMsg.getChat().getId(), uploadedGifVideoId,
								getUniqueId(),
								"with ref");
					...
					System.err.println(
							"================No Thumbinil Object in this Phot ===================");
				}
			...
			private void handleIncomingAudioMsg(IncomingMessage incomingMsg) {
				...
				System.out.println(
						"incomingMsg.getAudio().getId() : " + incomingMsg.getAudio().getId());
				System.out.println("incomingMsg.getAudio().getDuration() : "
						+ incomingMsg.getAudio().getDuration());
				System.out.println(
						"incomingMsg.getAudio().getTitle() : " + incomingMsg.getAudio().getTitle());
				System.out.println(
						"incomingMsg.getAudio().getSize(): " + incomingMsg.getAudio().getSize());
				System.out.println("incomingMsg.getAudio().getPerformer() : "
						+ incomingMsg.getAudio().getPerformer());
				System.out.println(
						"================start of Photo Thumbinil  Object ===================");

				AudioOutMessage audioOutMsg = new AudioOutMessage();
				audioOutMsg.setChatId(incomingMsg.getChat().getId());
				audioOutMsg.setReference(
						getUniqueId());
				...
				api.sendText(incomingMsg.getChat().getId(), "Audio Title : "
						+ incomingMsg.getAudio().getTitle() + " ,Audio Performer is : "
			...
			private void handleIncomingVoiceMsg(IncomingMessage incomingMsg) {
				System.out.println("================start of Voice Object ===================");
				System.out.println(
						"incomingMsg.getVoice().getId() : " + incomingMsg.getVoice().getId());
				System.out.println("incomingMsg.getVoice().getDuration(): "
						+ incomingMsg.getVoice().getDuration());
				System.out.println(
						"incomingMsg.getVoice().getSize(): " + incomingMsg.getVoice().getSize());
				System.out.println(
						"================start of Photo Thumbinil  Object ===================");

				VoiceOutMessage voiceOutMsg = new VoiceOutMessage();
				voiceOutMsg.setChatId(incomingMsg.getChat().getId());
				voiceOutMsg.setReference(
						getUniqueId());
				...
				api.sendText(incomingMsg.getChat().getId(), "Voice Size is : "
						+ incomingMsg.getVoice().getSize() + " and Voice Duration is :"
			...
			private void handleIncomingVideoMsg(IncomingMessage incomingMsg) {
				...
				System.out.println(
						"incomingMsg.getVideo().getId() : " + incomingMsg.getVideo().getId());
				System.out.println(
						"incomingMsg.getVideo().getWidth() : " + incomingMsg.getVideo().getWidth());
				System.out.println("incomingMsg.getVideo().getHeight() : "
						+ incomingMsg.getVideo().getHeight());
				System.out.println(
						"incomingMsg.getVideo().getSize() : " + incomingMsg.getVideo().getSize());
				System.out.println("incomingMsg.getVideo().getDuration() : "
						+ incomingMsg.getVideo().getDuration());
				System.out.println(
						"================start of Video Thumbinil  Object ===================");
				if (incomingMsg.getVideo().getThumbnail() != null) {
					System.out.println(
							"================End of Photo Thumbinil Object ===================");
				...
				api.sendText(incomingMsg.getChat().getId(),
						"Video Size is : " + incomingMsg.getVideo().getSize()
								+ " and Video width is :" + incomingMsg.getVideo().getWidth()
								+ " and Video height is :" + incomingMsg.getVideo().getHeight()
								+ " and Video duration is :"
								+ formatDurationInMinsAndSeconds(
										incomingMsg.getVideo().getDuration())
								+ " and caption is : " + incomingMsg.getCaption());
			...
			private void handleIncomingPhotoMsg(IncomingMessage incomingMsg) {
				...
				System.out.println(
						"incomingMsg.getPhoto().getId(): " + incomingMsg.getPhoto().getId());
				System.out.println(
						"incomingMsg.getPhoto().getWidth(): " + incomingMsg.getPhoto().getWidth());
				System.out.println("incomingMsg.getPhoto().getHeight(): "
						+ incomingMsg.getPhoto().getHeight());
				System.out.println(
						"incomingMsg.getPhoto().getSize(): " + incomingMsg.getPhoto().getSize());
				System.out.println(
						"================start of Photo Thumbinil  Object ===================");
				...
				MediaTransfer.downloadFile(TOKEN, incomingMsg.getPhoto().getId(), "./download",
						null);

				api.sendText(incomingMsg.getChat().getId(),
						"Photo Size is : " + incomingMsg.getPhoto().getSize()
								+ " and Photo width is :" + incomingMsg.getPhoto().getWidth()
								+ " and Photo height is :" + incomingMsg.getPhoto().getHeight()
								+ " and caption is : " + incomingMsg.getCaption()
